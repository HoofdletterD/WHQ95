...
public static class HeroSaveMapper
{
    public static HeroSaveData ToSaveData(IHero hero)
    {
        ...
        return new HeroSaveData
        {
            ...
            Bio = hero.Character.Bio ?? "",
            Traits = hero.Character.Traits.Select(t => t.ToString()).ToList()
        };
    }
    ...
    public static IHero FromSaveData(HeroSaveData data)
    {
        ...
        c.Bio = data.Bio ?? "";
        foreach (var traitString in data.Traits)
        {
            if (Enum.TryParse<HeroTrait>(traitString, out var trait))
                c.Traits.Add(trait);
        }
