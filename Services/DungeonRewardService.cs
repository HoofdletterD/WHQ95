...
using WHQCore.Libraries.MagicItems.TreasureCards;
using WHQCore.Logic;
...
namespace WHQCore.Services
{
    public static class DungeonRewardService
    {
        ...
        /// <summary>
        /// Rolls D666 and selects from the Dungeon Room Treasure Cards library.
        /// If no exact entry exists for the roll, it re-rolls until a valid item is found.
        /// </summary>
        private static void AwardTreasureCard(IHero hero)
        {
            MagicItemData? item = null;
            string d666 = string.Empty;

            while (item == null)
            {
                d666 = DiceRoller.RollD666();

                item = TreasureLibraryHelper.GetByD66(
                    typeof(DungeonRoomTreasureCardsLibrary),
                    d666
                );

                if (item == null)
                {
                    Console.WriteLine($"No exact D666 entry '{d666}' found in DungeonRoomTreasureCardsLibrary. Re-rolling...");
                }
            }

            hero.Character.MagicItems.Add(item);
            Console.WriteLine($"You drew a Treasure Card: {item.Name} (D666: {d666})");
