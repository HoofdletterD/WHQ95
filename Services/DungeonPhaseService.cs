...
public static class DungeonPhaseService
{
    public static void DungeonPhase(IHero heroBase)
    {
        ...
        while (true)
        {
            ...
            Console.WriteLine("0. Return to Hero Options");
            Console.Write("\nEnter your choice: ");
    ...
    private static void AddTreasureManually(IHero heroBase)
    {
        ...
        var allItems = TreasureLibraryHelper.GetAllItemsFromLibrary(typeof(MagicItemLibrary))
            ...
            .Concat(TreasureLibraryHelper.GetAllItemsFromLibrary(typeof(DungeonRoomTreasureCardsLibrary)))
        ...
        else
        {
            ...
            if (int.TryParse(selInput, out var selIndex) && selIndex >= 1 && selIndex <= allItems.Count)
            {
                ...
                if (selectedItem.StatModifiers.Any())
                    Console.WriteLine(
                        $"Stat Modifiers: {string.Join(", ", selectedItem.StatModifiers.Select(kv => $"{kv.Key} +{kv.Value}"))}");
                Console.WriteLine("--------------------");
