...
namespace WHQCompanion.HeroLevelUp;
...
public abstract class BaseHeroProgressionService : IHeroProgression
{
    ...
    protected abstract Spell GetSpellByRoll(string roll);
    ...
    protected virtual void ApplyRandomSpell(Hero character)
    {
        Spell newSpell = null;
        var existingValues = new HashSet<string>(character.Spells.Select(s => s.CastingNumber));
        var attempts = 0;
        ...
        do
        {
            ...
            var roll = DiceRoller.Roll2D6();
            newSpell = GetSpellByRoll(roll.ToString());
        } while (newSpell != null && existingValues.Contains(newSpell.SpellName));

        if (newSpell != null && !existingValues.Contains(newSpell.CastingNumber))
        {
            character.Spells.Add(newSpell);
            Console.WriteLine($"New skill gained: {newSpell.SpellName}");
        }
    }
